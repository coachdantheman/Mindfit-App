...
export default function FlowProgressView() {
  ...
  if (loading) return <p className="text-sm text-fg-4">Loading flow progress…</p>
    ...
    <div className="space-y-6">
      ...
      {(attention.length > 0 || recommendation) && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {attention.length > 0 ? (
            <div className="bg-surface rounded-2xl border border-orange-500/30 p-5">
              <p className="text-xs uppercase tracking-wide text-orange-400 font-semibold mb-2">Needs attention</p>
              <ul className="list-disc list-inside text-sm text-fg-2 space-y-1">
                {attention.map((r, i) => <li key={i}>{r}</li>)}
              </ul>
            </div>
          ) : (
            <div className="bg-surface rounded-2xl border border-green-500/30 p-5">
              <p className="text-xs uppercase tracking-wide text-green-400 font-semibold mb-2">Clean week</p>
              <p className="text-sm text-fg-2">No red flags. Keep compounding.</p>
            </div>
          )}
          <div className="bg-surface rounded-2xl border border-cta/30 p-5">
            ...
            <p className="text-sm text-fg-1">{recommendation}</p>
          </div>
        </div>
      )}
  ...
  return (
    <div className="card p-5">
      <div className="flex items-center justify-between mb-4">
        <p className="text-xs uppercase tracking-wide text-fg-4 font-semibold font-mono">Score breakdown</p>
        <span className="text-xs text-fg-4">out of 100</span>
      </div>
      <div className="space-y-2.5">
        {parts.map(p => (
          <div key={p.key} className="flex items-center gap-3">
            <span className="text-xs text-fg-3 w-28 shrink-0">{p.label}</span>
            ...
            <span className="text-sm font-semibold text-fg-2 w-12 text-right tabular-nums">
              {p.value}<span className="text-fg-4 text-xs">/{p.max}</span>
            </span>
          </div>
        ))}
      </div>
  ...
  return (
    <div className="card p-5">
      <div className="flex items-center justify-between mb-1">
        <h3 className="font-semibold text-fg-1 text-sm">Stage Distribution</h3>
        <span className="text-xs text-fg-4">last 30 days · {total} check-in{total === 1 ? '' : 's'}</span>
      </div>
      {total === 0 ? (
        <p className="text-sm text-fg-4 mt-3">No stage check-ins in the last 30 days.</p>
      ) : (
          ...
          {stages.map(s => {
            ...
            return (
              <div key={s.stage} className="flex items-center gap-3">
                <span className="text-xs text-fg-2 w-24 shrink-0">
                ...
                <span className="text-sm font-semibold text-fg-2 w-10 text-right tabular-nums">{s.pct}%</span>
              </div>
            )
          })}
        </div>
...
function RitualAdherenceCard({
  ...
  return (
    <div className="card p-5">
      <div className="flex items-center justify-between mb-1">
        <h3 className="font-semibold text-fg-1 text-sm">5A Ritual Adherence</h3>
        <span className="text-xs text-fg-4">{sessionCount} session{sessionCount === 1 ? '' : 's'}</span>
      </div>
      {sessionCount === 0 ? (
        <p className="text-sm text-fg-4 mt-3">No 5A sessions yet — run one before your next competition.</p>
      ) : (
        <>
          ...
          {(!worst || worst[1] < 30) && (
            <p className="text-xs text-green-400 mt-3">Ritual is holding up. No step is consistently getting cut.</p>
          )}
        </>
      )}
    </div>
...
function TriggerEffectivenessCard({
  ...
  return (
    <div className="card p-5">
      <div className="flex items-center justify-between mb-1">
        <h3 className="font-semibold text-fg-1 text-sm">Trigger Effectiveness</h3>
        <span className="text-xs text-fg-4">Avg flow score when fired</span>
      </div>
      <p className="text-xs text-fg-4 mb-4">Sorted by average flow score — top triggers drive your best performances.</p>
      <div className="space-y-2">
        {stats.map(s => (
          <div key={s.trigger} className="flex items-center gap-3">
            <span className="text-xs text-fg-2 w-40 shrink-0 truncate" title={TRIGGER_LABEL[s.trigger]}>
            ...
            <span className="text-sm font-semibold text-fg-2 w-14 text-right tabular-nums">
              {s.avgFlow.toFixed(1)}
              <span className="text-fg-4 text-[10px] ml-0.5">×{s.firedCount}</span>
            </span>
          </div>
        ))}
      </div>
