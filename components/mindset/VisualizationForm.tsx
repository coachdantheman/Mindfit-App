...
export default function VisualizationForm() {
  ...
  useEffect(() => {
    fetch(`/api/mindset/visualization?date=${today}`)
      .then(r => r.json())
      .then(data => {
        if (data.length > 0) {
  ...
  const handleSave = async () => {
    setSaving(true)
    const res = await fetch('/api/mindset/visualization', {
      method: 'POST',
  ...
  return (
    <div className="space-y-5">
      <div className="bg-gray-900 rounded-2xl border border-white/10 p-6">
        <h3 className="font-semibold text-gray-100 mb-1">Daily Visualization</h3>
        <p className="text-sm text-gray-500 mb-5">See yourself performing at your peak. Visualize the outcome you want before it happens.</p>
        ...
        <div className="space-y-4">
          <label className="flex items-center gap-3 cursor-pointer">
            ...
            <span className="text-gray-200 font-medium">I visualized today</span>
          </label>
          ...
          <div>
            ...
            <textarea
              ...
              placeholder="What did you visualize? How did it feel?"
              rows={3}
